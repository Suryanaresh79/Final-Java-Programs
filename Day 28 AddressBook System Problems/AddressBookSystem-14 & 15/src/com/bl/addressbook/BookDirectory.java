...
public class BookDirectory {
	...
	List<Contact> filterCity(String city) {
		...
		for (Map.Entry<String, AddressBook> books : mainAddressBook.entrySet()) {
			...
			List<Contact> filteredContacts = entryContact.stream().filter(person -> person.getClass().equals(city))
					.toList();
	...
	List<Contact> filterState(String state) {
		...
		for (Map.Entry<String, AddressBook> books : mainAddressBook.entrySet()) {
			List<Contact> entryContact = books.getValue().entry;
			List<Contact> filteredContacts = entryContact.stream().filter(person -> person.getState().equals(state))
					.toList();
			citizens.addAll(filteredContacts);
		}
		return citizens;
	...
	public String toString() {
		return "BookDirectory{" + "mainAddressBook=" + mainAddressBook + "\n" + '}';
	}
