...
//        addContact();
	...
	void handleSortChoice(int option) {
		switch (option) {
		case 1 -> {
			List<Contact> sorted = entry.stream().sorted(Comparator.comparing(Contact::getFirstName)).toList();
			sorted.forEach(people -> System.out.println(people + "\n"));
		}
		case 2 -> {
			List<Contact> sorted = entry.stream().sorted(Comparator.comparing(Contact::getCity)).toList();
			sorted.forEach(people -> System.out.println(people + "\n"));
		}
		case 3 -> {
			List<Contact> sorted = entry.stream().sorted(Comparator.comparing(Contact::getState)).toList();
			sorted.forEach(people -> System.out.println(people + "\n"));
		}
		case 4 -> {
			List<Contact> sorted = entry.stream().sorted(Comparator.comparing(Contact::getZip)).toList();
			sorted.forEach(people -> System.out.println(people + "\n"));
		}
		}
	}
	...
	void addContact() {
		...
		Contact obj4 = new Contact(4, "Manju", "Verma", "TP", "Mumbai", "Maharastra", "467077", "1245",
				"asd@gmail.com");
